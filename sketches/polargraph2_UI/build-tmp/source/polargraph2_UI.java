...
public class polargraph2_UI extends PApplet {
...
PFont dyFont;                      // DY UI 
//font = loadFont ("t132.ttf");  // DY UI 
//dyFont = createFont("t132.ttf");
//textFont(dyFont);                 // DY UI 

  // DY UI colors of functions
  int fc1 = 0xff7600FF;  // DY UI production
  int fc2 = 0xffFF00F3;  // DY UI entertainment
  int fc3 = 0xffC3FF00;  // DY UI service
  int fc4 = 0xffFF8D00;  // DY UI art
  int fc5 = 0xff00E3FF;  // DY UI education
  int fc6 = 0xff0028FF;  // DY UI restobars
  int fc7 = 0xff00FFA3;  // DY UI retail
  int fc8 = 0xff949B98;  // DY UI emtyspaces
  int fc9 = 0xff050505;  // DY UI roofs 
  int fx = 5;
...
public void setup(){
  ...
  // dyFont = loadFont ("t132.ttf",15,true); 
  dyFont = createFont("t132.ttf",15,true);
  // textFont(dyFont);                 // DY UI 
...
public void draw(){
  ...
  fill(fc1); text ("PRODUCTION",      fx,15); // DY UI
  fill(fc2); text ("ENTERTAINMENT",   fx,30); // DY UI
  fill(fc3); text ("SERVICE",         fx,45); // DY UI
  fill(fc4); text ("ART",             fx,60); // DY UI
  fill(fc5); text ("EDUCATION",       fx,75); // DY UI
  fill(fc6); text ("RESTOBARS",       fx,90); // DY UI
  fill(fc7); text ("RETAIL",          fx,105); // DY UI
  fill(fc8); text ("EMPTYSPACES",     fx,120); // DY UI
  fill(fc9); text ("ROOFSPACES",      fx,135); // DY UI
  fill(0xff03215F,30);
  rect(fx-2,3,107,137);
...
class ItemView{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "polargraph2_UI" };
    if (passedArgs != null) {
